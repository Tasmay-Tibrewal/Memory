...
def _try_get_native_nsa_parallel_fwd():
    ...
    try:
        ...
        try:
            ...
            candidates.append(here.parent / "native-sparse-attention" / "native_sparse_attention")
            candidates.append(here.parent / "native-sparse-attention-triton" / "native_sparse_attention")
            candidates.append(here / "native_sparse_attention")
            candidates.append(here.parent / "native_sparse_attention")
            for p in sys.path:
...
def qkv_kernel(
    ...
    forward_kernel_opt[grid_fwd](
        ...
        token_index_mapping,
        valid_topk_idx_permuted_tile,
        valid_topk_w_permuted_tile if has_weights else valid_topk_idx_permuted_tile,
        valid_lens,
        valid_start_indices,
        compute_min_block_id,
        cur_max_valid_tokens,
    ...
    for h in range(num_heads // head_tile):
        ...
        if h % gqa_deg == 0:
            if topk_w is None:
                ...
                topk_w_tile = topk_w[(h // gqa_deg) * head_tile: ((h // gqa_deg + 1)) * head_tile]
...
def _build_permute_results_per_seq_for_bwd(
    ...
    for kh in range(num_kv_heads):
        ...
        permute_results["valid_topk_idx_permuted_tile"].append(valid_topk_idx_permuted_tile)
        permute_results["valid_topk_w_permuted_tile"].append(
            topk_w_permuted_tile[mask_valid] if topk_w is not None else None
        )
        permute_results["valid_lens"].append(valid_lens)
        permute_results["valid_start_indices"].append(valid_start_indices)

        topk_idx_permuted_tile.fill_(-1)
        # Weight tile is only consumed under mask_valid; no need to clear.
...
def _topk_sparse_attention_fwd_nsa_style(
    ...
    def _torch_small_g_forward_chunk(
        ...
        bw_chunk: Optional[torch.Tensor] = None,  # [1, Tq, HK, S]
    ):
        """
        ...
        if bw_chunk is not None:
            w_ch = bw_chunk[0].to(torch.float32)                                  # [Tq, HK, S]
            w_tok = w_ch.unsqueeze(-1).expand(tqa, hk, s, block_size).reshape(tqa, hk, ksel)
    ...
    for i in range(len(cu_seqlens_q) - 1):
        ...
        if native_nsa_fwd is not None:
            ...
            if nsa_shape_ok and (t_mem == t_full) and (topk_w is None):
        ...
        prefix_mode = (
            q_len_seq > k_len_seq
            and bool((topk_idx_seq[:, 0:1, :] < 0).all().item())
        )
        if prefix_mode:
...
def backward_dq_opt_per_seq(
    ...
    Key optimizations vs the original local copy:
      ...
      4) Optional staging dtype control via `FSA_LOCAL_DQ_BUFFER_DTYPE`:
         - auto (default): fp16 if inputs are bf16; else input dtype
         - fp16 / bf16 / fp32
    """
    head_tile = 1
    ...
    global_max_valid_tokens = int(permute_results["global_max_valid_tokens"]) if num_blocks > 1 else int(
        permute_results["valid_lens_all"].max().item()
    )

    # No routed queries at all -> dQ is zero.
    max_tokens_any_block = int(permute_results["valid_lens_all"].max().item()) if "valid_lens_all" in permute_results else 0
    if max_tokens_any_block <= 0:
        return dq

    # Choose staging dtype (defaults to input dtype for numerical stability).
    ...
    token_index_mapping = torch.empty((head_tile, num_blocks, total_len), dtype=torch.int32, device=q.device)
    ...
    if total_len > total_len_k:
        ...
        prefix_mask_per_kh = [False] * num_k_heads
    ...
    for kh in range(num_k_heads):
        ...
        valid_lens_cpu = valid_lens.to(dtype=torch.int32, device="cpu").tolist()
        ...
        prefix_mode = bool(prefix_mask_per_kh[kh])
        if prefix_mode:
        ...
        # Iterate over the G query heads that share this KV head.
        for sh in range(num_share_q_heads):
            h = kh * num_share_q_heads + sh

            q_tile = q[:, h * head_tile:(h + 1) * head_tile]
            k_tile = k[:, kh * head_tile:(kh + 1) * head_tile]
            v_tile = v[:, kh * head_tile:(kh + 1) * head_tile]
            ...
            for compute_min_block_id in range(min(2, num_blocks)):
                if compute_min_block_id == 0:
                    compute_tile_size = 1
                    cur_max_valid_tokens = int(valid_lens_cpu[0]) if len(valid_lens_cpu) > 0 else 0
                    cur_valid_lens = valid_lens[0]
                    cur_valid_start_indices = valid_start_indices[0]
                    ...
                    if cur_valid_lens.numel() == 0:
                        continue
                    cur_max_valid_tokens = max(valid_lens_cpu[compute_min_block_id:])
...
def backward_dkdv(
    ...
    tq_slot_ptr,  # topk_q_slot: kh x N (slot id in [0, TOPK))
    tq_w_ptr,  # topk_q_w: kh x N (float32)
    lse_ptr,  # LSE: qh x n
    d_ptr,  # Delta: qh x n
    do_ptr,
    ...
    for i in range(0, act_q_len, BLOCK_SIZE_Q):
        ...
        dv += tl.dot(p.T, do)
        if HAS_WEIGHTS:
            # dL/d(log w_s) = sum_i p_i * (dp_i - d) = sum(ds)/sm_scale
            # dL/dw_s = (1/w_s) * dL/d(log w_s)
...
class FSATopkSparseAttentionWeighted(torch.autograd.Function):
    ...
    def forward(
        ...
        v: torch.Tensor,  # [total_len, num_k_heads, head_dim]
        topk_idx: torch.Tensor,  # [num_kv_heads, total_len, topk]
        topk_w: torch.Tensor,  # [num_kv_heads, total_len, topk]
        block_size: int,
        cu_seqlens_q: torch.Tensor,
        cu_seqlens_k: torch.Tensor,
        max_seqlen_q: torch.Tensor,
        ...
        assert q.dtype == k.dtype and k.dtype == v.dtype
        assert topk_idx.dtype == torch.int32
        assert topk_w.dtype in (torch.float16, torch.bfloat16, torch.float32)
        assert cu_seqlens_q.dtype == torch.int32 and cu_seqlens_k.dtype == torch.int32
        if sm_scale is None:
            sm_scale = 1 / math.sqrt(q.shape[-1])
...
def _expand_topk_w_for_internal_blocks(
    ...
    if ratio == 1:
        return topk_w
    # topk_w: [H, N, S] -> expanded: [H, N, S*ratio]
