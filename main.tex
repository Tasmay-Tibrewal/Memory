...
\mathbf{K} &\in \mathbb{R}^{B \times L \times H \times N_{\text{routed}} \times D}
\end{align}
...
\begin{enumerate}[noitemsep]
    \item Store only the unique chapters in memory: $\mathbf{K}_{\text{unique}} \in \mathbb{R}^{C_{\text{unique}} \times H \times N_c \times D}$
    \item Maintain a routing index that maps each query token to its assigned chapters
    \item Perform the attention computation by referencing the same chapter data for multiple queries
\end{enumerate}
...
\begin{itemize}[noitemsep]
    \item Long-context retention beyond the attention window
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Recency:} Newer information is retained longer (benefit of doubt for importance)
\end{itemize}
...
\textbf{Context Compression:} Activation Beacon \citep{activationbeacon2024} and IC-Former \citep{icformer2024} compress long contexts into shorter representations. Our context bank proposal incorporates similar compression ideas but maintains a persistent store across inference sessions with importance-weighted consolidation.
...
\textbf{Routing and Sparse Attention:} Routing Transformer \citep{roy2021routing} uses clustering for content-based sparse attention. MoE architectures \citep{shazeer2017moe, fedus2022switch} use token-wise routing to experts. Our chapter routing applies MoE-style routing to memory chapters---a novel application that enables scaling memory banks while maintaining attention-based access.
...
\newblock \emph{arXiv preprint arXiv:2405.06067}.
...
Borgeaud, S., Mensch, A., Hoffmann, J., Cai, T., Rutherford, E., Millican, K., van den Driessche, G., Lespiau, J.-B., Damoc, B., Clark, A., de Las Casas, D., Guy, A., Menick, J., Ring, R., Hennigan, T., Huang, S., Maggiore, L., Jones, C., Cassirer, A., Brock, A., Paez, P., Sheridan, G., Landon, J., and Sifre, L. (2022).
